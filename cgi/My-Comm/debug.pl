...
sub get_all_member_profiles{
   my ( $name ) = @_;
   my @profiles = get_all_member_profiles_from_db ($name);
   return @profiles;
}


sub get_user_f_and_l_name_by_uname {
   my ( $uname ) = @_;
   my ($fname,$lname) = get_user_f_and_l_name_by_uname_from_db( $uname );
   return ($fname,$lname);
}

sub get_number_of_person_in_apartment {
   my ( $name ) = @_;
   my $ret = get_number_of_person_in_apartment_from_db( $name );
   return $ret;
}   

sub  update_member_profile_data{
      my ( %hash ) = @_;
      my $ret = update_member_profile_data_to_db ( %hash );
      return $ret;
}

sub modify_member_profile_data {
   my ( %hash ) = @_;
   my $ret = modify_member_profile_data_to_db ( %hash );
   return $ret;
}

sub get_max_index_of_member_ids{
   my ( $name ) = @_;
   my @member_ids = get_all_member_ids_of_apartment_from_db ( $name);  
   my @indexes;
   my $pattern = $name . '_' ;  
   foreach  ( @member_ids ){
      my $member_id = $_;
      $member_id =~ s/$pattern//g;
      push (@indexes,$member_id);
   }
   @indexes = sort { $a <=> $b } @indexes;
   my $max = $indexes[$#indexes];
   return $max;
}

sub get_user_passwd_by_name{
    my ($uname) = @_;
    my $passwd = get_user_passwd_by_name_from_db ($uname);
    return $passwd;
}

sub delete_member_profile_by_member_id {
    my ( $member_id ) = @_;
    my $ret = delete_member_profile_by_member_id_from_db ( $member_id);
    return $ret;
}

sub get_member_profile_by_member_id{
    my ( $member_id ) = @_;
    my %member_profile = get_member_profile_by_member_id_from_db ( $member_id);
    return %member_profile;
}

sub get_unames_in_block_floor{
   my ($pat ) = @_;   
   my @unames = get_unames_in_block_floor_from_db ( $pat);
   return @unames; 
}

sub get_locked_unames_in_block_floor{
   my ($pat ) = @_;
   my @unames = get_locked_unames_in_block_floor_from_db( $pat);
   return @unames;
}

sub update_message{
   my (%hash) = @_;
   my $ret = update_message_to_db (%hash);
   return $ret;
}

sub get_user_mail_list_by_uname {
    my ( $uname ) = @_;
    my @mail_list = get_user_mail_list_by_uname_from_db ( $uname );
    return  @mail_list;
}

sub get_message_by_msg_id {
    my ($msg_id, $uname) = @_;
    my %data = get_message_by_msg_id_from_db ( $msg_id, $uname);
    return %data; 
}

sub delete_message_by_msg_id {
   my ( $msg_id, $uname ) = @_;
   my $ret = delete_message_by_msg_id_from_db ($msg_id, $uname );
   return $ret;

}

sub get_user_sentmail_list_by_uname {
   my ( $uname ) = @_ ;
   my @list = get_user_sentmail_list_by_uname_from_db ($uname); 
   return @list;
}

sub get_delete_msg_list_from_inbox_by_uname{
   my ( $uname ) = @_ ;
   my @list = get_delete_msg_list_from_inbox_by_uname_from_db ($uname); 
   return @list;
   
}

sub get_deleted_sentmail_list_by_uname{
    my ( $uname ) = @_ ;
    my @list = get_deleted_sentmail_list_by_uname_from_db ($uname); 
    return @list;
}

sub get_sent_message_by_msg_id {
   my( $msg_id, $uname ) = @_;
   my %hash = get_sent_message_by_msg_id_from_db ( $msg_id, $uname );
   return %hash;
}

sub delete_sent_message_by_msg_id {
      my ( $msg_id, $uname ) = @_;
      my $ret = delete_sent_message_by_msg_id_from_db  ( $msg_id, $uname );
      return $ret;
}

sub get_all_unames{
    my @unames = get_all_unames_from_db ();
    return @unames ;
}

sub search_pattern_from_profiles {
    my ( $search ) = @_;
    my @unames = search_pattern_from_profiles_from_db ( $search );
    return @unames;
}

sub get_broadcast_msg_count {
    my ( $uname ) = @_;
    my $count = get_broadcast_msg_count_from_db ($uname);
    return $count;
}


sub get_unread_msg_count {
   my ($name ) = @_;
   my $count = get_unread_msg_count_from_db ($name);
   return $count;
}
sub update_owner_info {
   my( %hash) = @_;
   my $ret = update_owner_info_to_db ( %hash);
   return $ret;  
}   

sub modify_owner_info {
   my( %hash) = @_;
   my $ret = modify_owner_info_to_db ( %hash);
   return $ret;  
}   
...
sub get_owner_info{
   my ( $uname ) = @_;
   my %hash = get_owner_info_from_db ( $uname);
   return %hash;
}   
   
sub change_user_passwd {
   my( $uname, $passwd ) = @_;
   my $sql = "update user_index set password=\'$passwd\' where uname=\'uname\';";
   my $ret = my_sql_exec ( $sql );
   return $ret;
}   
   
sub submit_admin_notice_preview_text {
   my ($text) = @_;
   my $sql = "update admin_notice_preview set my_text=\'$text\';";
   my $ret = my_sql_exec ( $sql );
   return $ret;
}

sub get_admin_notice_preview_text{
    my $html_text = get_admin_notice_preview_text_from_db ( );
    return $html_text;
}

sub get_admin_notice_text{
   my $notice_html = get_admin_notice_text_from_db ();
   return $notice_html;
   
}
sub promote_preview_notice {
    my $ret = promote_preview_notice_from_db ();
    return $ret;
}

sub get_all_notice_list{
   my @notice_list = get_all_notice_list_from_db ();
   return @notice_list;
}

sub get_admin_notice_by_notice_id {
    my ($notice_id) = @_;
    my $html_txt = get_admin_notice_by_notice_id_from_db ( $notice_id );
    return $html_txt;
} 

sub delete_notice_by_id {
   my( $notice_id ) = @_;
   my $sql = "update notice_info set delete_flag=\'1\' where notice_id=\'$notice_id\';";
   my $ret = my_sql_exec ( $sql );
   return $ret;
}

sub delete_existing_notice{
   my $sql = "update notice_info set active_flag=\'0\' where active_flag=\'1\';";
   my $ret = my_sql_exec ( $sql );
   $sql = "update notice_info set active_flag=\'1\' where notice_id=\'xxx0xxx\';";
   $ret = my_sql_exec ( $sql );
}

sub create_sudo_admin{
     my ($uname) = @_;
     my $sql = "update user_index set user_role=\'2\' where uname=\'$uname \';";
     my $ret = my_sql_exec ( $sql );
     return $ret;
}

sub depromote_sudo_admin{
     my ($uname) = @_;
     my $sql = "update user_index set user_role=\'0\' where uname=\'$uname \';";
     my $ret = my_sql_exec ( $sql );
     return $ret;
}
...
sub register_admin{
   ...
   my $sql = "insert into user_index 
                                 (
                                       user_role,  uname,   password,   fname,
                                       lname,      email ,     mobile, active
                                 )                                            
                              values
                                  (
                                    \'3\' , \'$params{uname}\' ,  \'$params{passwd1}\' , \'$params{fname}\',
                                    \'$params{lname}\' , \'$params{email}\' , \'$params{mobile}\', \'1\'
                                  );
             ";
     my $ret = my_sql_exec ( $sql );
     return $ret;
...
sub get_admin_and_sudo_admin_info{
      my @admin_list = get_admin_and_sudo_admin_info_from_db ();
      return @admin_list;
}

sub delete_admin {
   my ($uname) = @_;
   my $ret = delete_admin_from_db ($uname);
   return $ret;
}

sub lock_occupant_account {
    my ($uname) = @_;
    my $sql = "update user_index set
                                  active = \'-1\'
                              where uname=\'$uname\';                         
...
sub delete_all_msgs_from_user {
    my ($uname) = @_;
    delete_all_message_by_uname_from_db ($uname);
    delete_all_sent_message_by_uname_from_db ($uname);
    return 1;   
}

sub back_up_member_profile_by_uname{
            my ($uname) = @_;
            my $ret = back_up_member_profile_by_uname_from_db ($uname);
            return 1;
}

sub delete_up_member_profile_by_uname{
    my ($uname) = @_;
    my $sql = "DELETE FROM member_profile
                                       where
                                       uid = (
...
sub get_admin_msg_count {
    my ($uname) = @_;
    my $count = get_admin_msg_count_from_db($uname);
    return $count;
}
####################################################################################
...
sub get_all_subscribed_emails{
   my ($mpid,$spid) = @_;
   my @emails = get_all_subscribed_emails_db ($mpid,$spid);
   return @emails;
}
...
sub verify_old_passwd {
   my ( $name, $old_passwd  ) = @_;
   my $passwd = get_user_passwd_by_name ( $name );
   if ( $passwd eq $old_passwd){
      return 1;
   }else{
      return 0;
   }
...
sub get_user_email_by_name {
    my ($uname) = @_;
    my $email =  get_user_email_by_name_from_db ( $uname);
    return $email;
}

sub get_name_by_email {
   my ($email) = @_;
    my $uname =  get_name_by_email_from_db ( $email);
    return $uname;  
}

sub get_mpid_spid_by_qid{
   my ($qid) = @_;
   my($mpid,$spid) = get_mpid_spid_by_qid_db ($qid);
   return ($mpid,$spid);   
}

sub get_sub_topic_name {
   my( $mpid, $spid ) = @_;
   my $sub_topic = get_sub_topic_name_db ( $mpid, $spid );
   return $sub_topic;
}   

sub get_quesation_by_qid{
   my ($qid) = @_;
   my $q = get_quesation_by_qid_db ( $qid);
   return $q; 
}

sub get_all_discussion {
   my ($qid) = @_;
   my @discussions = get_all_discussion_db ($qid);     
   return @discussions; 
}
...
sub get_qid_n_qtxt{
   my ($mpid,$spid,$uid) = @_;
   my @Qlist = get_qid_n_qtxt_db ( $mpid,$spid,$uid) ;   
   return @Qlist;
}

sub add_new_quesation {
   my ($sql) = @_;
   my $ret = my_sql_exec ( $sql );
   return $ret;
}

sub get_main_page_info{
    my @main_page_info = get_main_page_info_db ();
    return @main_page_info;
   
}

sub get_all_subpage_info_by_mpid {
    my($mpid,$uid) = @_;
    my @sub_page_info = get_all_subpage_info_by_mpid_db ($mpid,$uid);
...
sub subscribe_user{
   ...
   my $ret = subscribe_user_db ( $mpid, $spid, $uid , $status );
   return $ret;
}

sub get_sub_topic_name_by_spid {
   my ( $spid) =  @_;
   my $sub_topic_name = get_sub_topic_name_by_spid_db ( $spid);
   return $sub_topic_name;
}   

sub get_main_topic_name_by_mpid {
   my ($mpid) = @_;
   my $main_topic_name = get_main_topic_name_by_mpid_db ( $mpid);
   return $main_topic_name;
}   

sub is_user_subscribed {
   my ( $mpid, $spid, $uid ) = @_;
   my $ret =  is_user_subscribed_db ( $mpid, $spid, $uid );
   return $ret;
}   
...
sub get_uname_by_FL_names {
   my ( $fname , $lname) = @_;
   my $uname = get_uname_by_FL_names_from_db ( $fname , $lname );
   return $uname;
}

sub  get_all_login_fnames{
   my @ret = get_all_login_fnames_from_db ();
    return @ret;
}

sub get_all_login_lnames{
  my @ret =  get_all_login_lnames_from_db();
  return @ret;
}
...
sub delete_reserve_by_book_id{
   my ( $sql ) = @_;
   my $ret = my_sql_exec ( $sql );
   return $ret;
}

sub insert_trial_usr_info {
   my ( $sql ) = @_;
   my $ret = my_sql_exec ( $sql );
   return $ret;   
